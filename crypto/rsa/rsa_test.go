...
func TestKeyGeneration(t *testing.T) {
	...
	for _, size := range sizes {
		t.Run(fmt.Sprintf("%d", size), func(t *testing.T) {
			if size < 1024 {
				// excrypto: fork accepts small keys; skip GODEBUG-rejection check.
				t.Skip("excrypto: fork accepts small RSA keys without GODEBUG")
			}
...
func TestKeyTooSmall(t *testing.T) {
	t.Skip("excrypto: fork accepts small RSA keys by design (permissive parsing for scanning)")
}
