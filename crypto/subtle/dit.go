...
// excrypto: out-of-tree fork cannot link to runtime-internal DIT helpers, so
// these are local no-ops. Production callers must enable DIT via the host Go
// runtime; here we just return false (not enabled) and skip disable.
func setDITEnabled() bool { return false }
