...
var AllowedInternalPackages = map[string]bool{
	...
	"github.com/runZeroInc/excrypto/crypto/internal/entropy":        true,
	"github.com/runZeroInc/excrypto/crypto/internal/entropy/v1.0.0": true,
	"github.com/runZeroInc/excrypto/crypto/internal/sysrand":        true,

	// impl.Register is how the packages expose their alternative
	// implementations to tests outside the module.
	"github.com/runZeroInc/excrypto/crypto/internal/impl": true,

	// randutil.MaybeReadByte is used in non-FIPS mode by GenerateKey functions.
	"github.com/runZeroInc/excrypto/crypto/internal/randutil": true,

	// constanttime are the constant-time intrinsics.
	"github.com/runZeroInc/excrypto/crypto/internal/constanttime": true,
}
...
{{end -}}`, "github.com/runZeroInc/excrypto/crypto/internal/fips140/...")
	...
	for _, line := range strings.Split(out, "\n") {
		...
		if strings.Contains(importedPkg, "github.com/runZeroInc/excrypto/crypto/internal") {
