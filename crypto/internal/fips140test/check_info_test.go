...
// excrypto: this test relies on abi.FuncPCABIInternal, which is a compiler
// intrinsic only resolvable when the package lives under GOROOT. Out-of-tree
// the linker reports "relocation target internal/abi.FuncPCABIInternal not
// defined", so the entire test binary fails to link. Gate the test behind a
// build tag we never set, leaving the rest of the fipstest suite usable.

//go:build excrypto_fips_integrity_info
...
import (
	...
	"github.com/runZeroInc/excrypto/internal/abi"
)
