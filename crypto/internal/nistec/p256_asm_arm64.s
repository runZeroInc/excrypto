...
TEXT p256SubInternalExCrypto<>(SB),NOSPLIT,$0
	...
	RET
/* ---------------------------------------*/
TEXT p256SqrInternalExCrypto<>(SB),NOSPLIT,$0
	// x[1:] * x[0]
	MUL	x0, x1, acc1
	UMULH	x0, x1, acc2
	...
	RET
/* ---------------------------------------*/
TEXT p256MulInternalExCrypto<>(SB),NOSPLIT,$0
	// y[0] * x
	MUL	y0, x0, acc0
	UMULH	y0, x0, acc1
...
TEXT ·p256PointAddAffineAsm(SB),0,$264-48
	...
	// Begin point add
	LDx(z1in)
	CALL	p256SqrInternalExCrypto<>(SB)    // z1ˆ2
	STy(z1sqr)

	LDx(x2in)
	CALL	p256MulInternalExCrypto<>(SB)    // x2 * z1ˆ2

	LDx(x1in)
	CALL	p256SubInternalExCrypto<>(SB)    // h = u2 - u1
	STx(h)

	LDy(z1in)
	CALL	p256MulInternalExCrypto<>(SB)    // z3 = h * z1
	...
	LDy(z1sqr)
	CALL	p256MulInternalExCrypto<>(SB)    // z1 ^ 3

	LDx(y2in)
	CALL	p256MulInternalExCrypto<>(SB)    // s2 = y2 * z1ˆ3
	STy(s2)

	LDx(y1in)
	CALL	p256SubInternalExCrypto<>(SB)    // r = s2 - s1
	STx(r)

	CALL	p256SqrInternalExCrypto<>(SB)    // rsqr = rˆ2
	STy	(rsqr)

	LDx(h)
	CALL	p256SqrInternalExCrypto<>(SB)    // hsqr = hˆ2
	STy(hsqr)

	CALL	p256MulInternalExCrypto<>(SB)    // hcub = hˆ3
	STy(hcub)

	LDx(y1in)
	CALL	p256MulInternalExCrypto<>(SB)    // y1 * hˆ3
	STy(s2)
	...
	CALL	p256MulInternalExCrypto<>(SB)    // u1 * hˆ2
	STP	(y0, y1), h(0*8)
	STP	(y2, y3), h(2*8)
	...
	LDy(rsqr)
	CALL	p256SubInternalExCrypto<>(SB)    // rˆ2 - u1 * hˆ2 * 2
...
TEXT ·p256PointAddAsm(SB),0,$392-32
	...
	// Begin point add
	LDx(z2in)
	CALL	p256SqrInternalExCrypto<>(SB)    // z2^2
	STy(z2sqr)

	CALL	p256MulInternalExCrypto<>(SB)    // z2^3

	LDx(y1in)
	CALL	p256MulInternalExCrypto<>(SB)    // s1 = z2ˆ3*y1
	STy(s1)

	LDx(z1in)
	CALL	p256SqrInternalExCrypto<>(SB)    // z1^2
	STy(z1sqr)

	CALL	p256MulInternalExCrypto<>(SB)    // z1^3

	LDx(y2in)
	CALL	p256MulInternalExCrypto<>(SB)    // s2 = z1ˆ3*y2
	...
	LDx(z2sqr)
	LDy(x1in)
	CALL	p256MulInternalExCrypto<>(SB)    // u1 = x1 * z2ˆ2
	STy(u1)

	LDx(z1sqr)
	LDy(x2in)
	CALL	p256MulInternalExCrypto<>(SB)    // u2 = x2 * z1ˆ2
	STy(u2)
	...
	LDx(z1in)
	LDy(z2in)
	CALL	p256MulInternalExCrypto<>(SB)    // z1 * z2
	LDx(h)
	CALL	p256MulInternalExCrypto<>(SB)    // z1 * z2 * h
	MOVD	res+0(FP), b_ptr
	STy(z3out)

	LDx(hsqr)
	LDy(u1)
	CALL	p256MulInternalExCrypto<>(SB)    // hˆ2 * u1
	STy(u2)
