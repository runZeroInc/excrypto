...
type rsaKeyAgreement struct {
	// excryptoLog: handshake-logging side-channel populated during
	// key exchange. Not used by the protocol logic.
	excryptoServerPub    *rsa.PublicKey // server cert's RSA public key (from generateClientKeyExchange)
	excryptoClientCipher []byte         // client-encrypted pre-master secret (without the 2-byte length prefix)
	excryptoClientPMS    []byte         // pre-master secret generated by client
	excryptoServerPMS    []byte         // pre-master secret recovered by server (processClientKeyExchange)
}
...
func (ka *rsaKeyAgreement) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	ka.excryptoClientCipher = append([]byte(nil), ciphertext...)
	ka.excryptoServerPMS = append([]byte(nil), preMasterSecret...)
	if rsaPub, ok := cert.PrivateKey.(*rsa.PrivateKey); ok {
		ka.excryptoServerPub = &rsaPub.PublicKey
	}
...
func (ka *rsaKeyAgreement) generateClientKeyExchange(config *Config, clientHello *clientHelloMsg, cert *x509.Certificate) ([]byte, *clientKeyExchangeMsg, error) {
	...
	ka.excryptoServerPub = rsaKey
	ka.excryptoClientCipher = append([]byte(nil), encrypted...)
	ka.excryptoClientPMS = append([]byte(nil), preMasterSecret...)
...
type ecdheKeyAgreement struct {
	...
	// excryptoLog: handshake-logging side-channel populated during
	// key exchange. Not used by the protocol logic.
	excryptoServerPub  []byte // server's ECDHE public key bytes (from SKX)
	excryptoClientPub  []byte // client's ECDHE public key bytes (CKX)
	excryptoSig        []byte // raw signature bytes from SKX
	excryptoSignedData []byte // bytes that were signed/verified
	excryptoSigType    uint8  // signaturePKCS1v15 / signatureECDSA / signatureEd25519 / signatureRSAPSS
	excryptoSigHash    crypto.Hash
	excryptoSigValid   bool  // true iff signature verified successfully
	excryptoSigError   error // non-nil if verification failed
}
...
func (ka *ecdheKeyAgreement) generateServerKeyExchange(config *Config, cert *Certificate, clientHello *clientHelloMsg, hello *serverHelloMsg) (*serverKeyExchangeMsg, error) {
	...
	ka.excryptoServerPub = append([]byte(nil), ecdhePublic...)
	...
	if ka.version >= VersionTLS12 {
		...
		ka.excryptoSignedData = append([]byte(nil), signed...)
		ka.excryptoSigType = sigType
		ka.excryptoSigHash = sigHash
		ka.excryptoSigValid = true
	} else {
...
func (ka *ecdheKeyAgreement) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	ka.excryptoClientPub = append([]byte(nil), ckx.ciphertext[1:]...)
...
func (ka *ecdheKeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	...
	ka.excryptoClientPub = append([]byte(nil), ourPublicKey...)
	...
	if ka.version >= VersionTLS12 {
		...
		ka.excryptoServerPub = append([]byte(nil), publicKey...)
		ka.excryptoSig = append([]byte(nil), sig...)
		ka.excryptoSignedData = append([]byte(nil), signed...)
		ka.excryptoSigType = sigType
		ka.excryptoSigHash = sigHash
		if err := verifyHandshakeSignature(sigType, cert.PublicKey, sigHash, signed, sig); err != nil {
			ka.excryptoSigError = err
