...
func mixround(r []uint16, kx []uint16, j *uint) {
	...
	x = r[0] + (r[1] & (^r[3])) + (r[2] & r[3]) + kx[*j]
	*j++
	r[0] = (x << 1) | (x >> 15)
	x = r[1] + (r[2] & (^r[0])) + (r[3] & r[0]) + kx[*j]
	*j++
	r[1] = (x << 2) | (x >> 14)
	x = r[2] + (r[3] & (^r[1])) + (r[0] & r[1]) + kx[*j]
	*j++
	r[2] = (x << 3) | (x >> 13)
	x = r[3] + (r[0] & (^r[2])) + (r[1] & r[2]) + kx[*j]
	*j++
	r[3] = (x << 5) | (x >> 11)
}
...
func rmixround(r []uint16, kx []uint16, j *uint) {
	...
	x = (r[3] << 11) | (r[3] >> 5)
	r[3] = x - ((r[0] & (^r[2])) + (r[1] & r[2]) + kx[*j])
	*j--
	x = (r[2] << 13) | (r[2] >> 3)
	r[2] = x - ((r[3] & (^r[1])) + (r[0] & r[1]) + kx[*j])
	*j--
	x = (r[1] << 14) | (r[1] >> 2)
	r[1] = x - ((r[2] & (^r[0])) + (r[3] & r[0]) + kx[*j])
	*j--
	x = (r[0] << 15) | (r[0] >> 1)
	r[0] = x - ((r[1] & (^r[3])) + (r[2] & r[3]) + kx[*j])
	*j--
}
