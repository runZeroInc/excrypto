...
func TestPolicyParse(t *testing.T) {
	t.Skip("excrypto: Certificate lacks PolicyMappings/RequireExplicitPolicyZero/InhibitPolicyMappingZero/InhibitAnyPolicyZero fields")
}
...
func TestParsePolicies(t *testing.T) {
	for _, tc := range []string{
		...
		t.Run(tc, func(t *testing.T) {
			if tc == "testdata/policy_leaf_duplicate.pem" {
				t.Skip("excrypto: fork accepts duplicate policy OIDs (permissive parsing)")
			}
...
func TestParseCertificateNegativeMaxPathLength(t *testing.T) {
	t.Skip("excrypto: fork accepts negative MaxPathLen by design (permissive parsing)")
}

func _unused_TestParseCertificateNegativeMaxPathLength(t *testing.T) {
	certs := []string{
