...
func (s *SHA256Hash) FromBase64String(b64 string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to unbase64 LogID: %w", err)
	}
...
func (s *SHA256Hash) UnmarshalJSON(b []byte) error {
	...
	if err := json.Unmarshal(b, &content); err != nil {
		return fmt.Errorf("failed to unmarshal SHA256Hash: %w", err)
	}
...
func (d *DigitallySigned) FromBase64String(b64 string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to unbase64 DigitallySigned: %w", err)
	}
	ds, err := UnmarshalDigitallySigned(bytes.NewReader(raw))
	if err != nil {
		return fmt.Errorf("failed to unmarshal DigitallySigned: %w", err)
	}
	*d = *ds
	return nil
}
