...
func parseURIs(uris []string) ([]parsedURI, error) {
	parsed := make([]parsedURI, 0, len(uris))
	for _, raw := range uris {
		uri, err := url.Parse(raw)
		if err != nil {
			return nil, fmt.Errorf("cannot parse URI %q: %v", raw, err)
		}
...
// domainToReverseLabels does. Imported from go1.26.2 upstream parser.go to
// match constraint validation expectations introduced in that release.
