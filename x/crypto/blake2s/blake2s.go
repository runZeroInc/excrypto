...
func (d *digest) MarshalBinary() ([]byte, error) {
	if d.keyLen != 0 {
		return nil, errors.New("github.com/runZeroInc/excrypto/stdlib/crypto/blake2s: cannot marshal MACs")
	}
...
func (d *digest) UnmarshalBinary(b []byte) error {
	if len(b) < len(magic) || string(b[:len(magic)]) != magic {
		return errors.New("github.com/runZeroInc/excrypto/stdlib/crypto/blake2s: invalid hash state identifier")
	}
	if len(b) != marshaledSize {
		return errors.New("github.com/runZeroInc/excrypto/stdlib/crypto/blake2s: invalid hash state size")
	}
	b = b[len(magic):]
	for i := 0; i < 8; i++ {
