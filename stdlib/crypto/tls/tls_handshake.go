...
import (
	...
	"github.com/runZeroInc/excrypto/stdlib/crypto/x509/ct"
)
...
type TLSVersionRaw uint16

type CipherSuiteRaw uint16
...
func (cs *CipherSuite) UnmarshalJSON(b []byte) error {
	...
	*cs = CipherSuite{ID: aux.Value, Name: aux.Name}
	return nil
}
...
func (m *clientHelloMsg) MakeLog() *ClientHello {
	...
	for i, aCipher := range m.cipherSuites {
		ch.CipherSuites[i] = CipherSuite{ID: aCipher}
	}
	...
	ch.SecureRenegotiation = len(m.secureRenegotiation) > 0
	...
	//ch.NextProtoNeg = m.nextProtoNeg // Not implemented
...
func (m *serverHelloMsg) MakeLog() *ServerHello {
	...
	sh.CipherSuite = CipherSuite{ID: m.cipherSuite}
	...
	sh.SecureRenegotiation = len(m.secureRenegotiation) > 0
	...
	// sh.NextProtoNeg = m.nextProtoNeg // Not implemented
	if len(m.scts) > 0 {
...
func (m *ClientSessionState) MakeLog() *SessionTicket {
	...
	st.Length = m.sessionTicket.Length
	st.Value = make([]uint8, st.Length)
	copy(st.Value, m.sessionTicket.Value)
...
func (m *serverHandshakeState) MakeLog() *KeyMaterial {
	...
	keymat.PreMasterSecret = new(PreMasterSecret)
	keymat.PreMasterSecret.Length = m.preMasterSecret.Length
	keymat.PreMasterSecret.Value = make([]byte, len(m.preMasterSecret.Value))
	copy(keymat.PreMasterSecret.Value, m.preMasterSecret.Value)
